 ...
 * @author Fuzhong.Yan
 */
public abstract class AZ_ClassLoader extends ClassLoader {
    /**
     * 类加载器的名字
     */
    private String name;
    ...
    public AZ_ClassLoader(String name) {
        super();//让系统加载器成为该类的加载器的父类加载器
        this.name = name;
    }

    public AZ_ClassLoader(String name, String path) {
        super();//让系统加载器成为该类的加载器的父类加载器
        this.name = name;
        this.path = path;
    }

    public AZ_ClassLoader(SecurityCoder securityCoder, String name) {
        super(); // 让系统加载器成为该类的加载器的父类加载器
        this.name = name;
        this.securityCoder = securityCoder;
    }

    public AZ_ClassLoader(SecurityCoder securityCoder, String name, String path) {
        super(); // 让系统加载器成为该类的加载器的父类加载器
        this.securityCoder = securityCoder;
        this.name = name;
        this.path = path;
    ...
    public AZ_ClassLoader(ClassLoader parent, String name) {
        super(parent); // 显示指定该类加载器的父加载器
        this.name = name;
    }

    public AZ_ClassLoader(ClassLoader parent, String name, String path) {
        super(parent); // 显示指定该类加载器的父加载器
        this.name = name;
        this.path = path;
    }

    public AZ_ClassLoader(ClassLoader parent, SecurityCoder securityCoder, String name) {
        super(parent); // 显示指定该类加载器的父加载器
        this.name = name;
        this.securityCoder = securityCoder;
    }

    public AZ_ClassLoader(ClassLoader parent, SecurityCoder securityCoder, String name, String path) {
        super(parent); // 显示指定该类加载器的父加载器
        this.name = name;
        this.securityCoder = securityCoder;
        this.path = path;
