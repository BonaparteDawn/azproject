 ...
 */
@Service
public class AZ_MenuService implements AZ_MenuApi, Serializable {
    @Autowired
    ...
    public AZ_MenuVo getOpenedMenus(Integer parent_id) throws Exception {
        Menu menu = new Menu();
        menu.setIs_open(AZ_Constant.MENU_OPEN);
    ...
    public List<Menu> getMenus(Menu menu) throws Exception {
        ...
        if (ObjectUtils.isNotEmpty(menu)) {
            ...
            if (ObjectUtils.isNotEmpty(menu.getParent_id())) {
                mc.andParent_idEqualTo(menu.getParent_id());
            }
            ...
            if (ObjectUtils.isNotEmpty(menu.getIs_open())) {
                mc.andIs_openEqualTo(menu.getIs_open());
            }
     ...
     * @param menu
     * @return
     * @throws Exception <br/>PARENT_MENU_DO_NOT_EXIST:父菜单不存在
    ...
    public boolean addMenu(Menu menu) throws Exception {
        ...
        if (ObjectUtils.isNotEmpty(menu)) {
            if (ObjectUtils.isNotEmpty(menu.getParent_id())) {
                Menu parentMenu = menuMapper.selectByPrimaryKey(menu.getParent_id());
     ...
     * @param menu_id
     * @return
     * @throws Exception <br/>MENU_CLOSED:菜单关闭
    ...
    public boolean closeMenu(int menu_id) throws Exception {
        ...
        if (ObjectUtils.isNotEmpty(menu)) {
            if (menu.getIs_open() == AZ_Constant.MENU_CLOSE) {
                throw new Exception("MENU_CLOSED");
            } else {
                menu.setIs_open(AZ_Constant.MENU_CLOSE);
     ...
     * @param menu_id
     * @return
     * @throws Exception <br/>MENU_DO_NOT_EXIST:菜单不存在
     */
    ...
    public boolean modifyMenu(Menu vo) throws Exception {
        ...
        if (ObjectUtils.isNotEmpty(vo)) {
            ...
            if (ObjectUtils.isNotEmpty(menu)) {
                Menu parentMenu = menuMapper.selectByPrimaryKey(vo.getParent_id());
                if (ObjectUtils.isEmpty(parentMenu)) {
                    throw new Exception("PARENT_MENU_DO_NOT_EXIST");
                }
     ...
     * @param child
     * @return
     * @throws Exception <br/>PARENT_ID_EMPTY:父菜单不存在
     */
    ...
    private AZ_MenuVo findMenuFamily(Integer parent_id, List<AZ_MenuVo> menus) throws Exception {
        ...
        if (parent_id != null && CollectionUtils.isNotEmpty(menus)) {
            for (int i = 0; i < menus.size(); i++) {
                ...
                if (temp.getParent_id() != null && temp.getParent_id() == parent_id) {
                    menus.remove(temp);
